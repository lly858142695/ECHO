...
## ECHO Developer 计划正在招募

ECHO NEXT 不是一个只靠公开仓库就能持续推进的播放器项目。当前主仓库保留源码、文档和公开构建信息，真正活跃的下一代开发会在开发者仓库、内部讨论和更早期的测试环境里推进。我们欢迎愿意长期参与的人加入 [ECHO Developer 计划](https://echonext.moe/zh/developer/)：你可以接触更早的功能设计、开发计划、实验分支和测试反馈，也可以直接参与 ECHO NEXT 的架构、播放链路、界面体验、插件生态、Linux 适配和文档维护。

我们尤其需要 **美工 / UI 视觉 / 图标 / 插画 / 动效方向的协作者**。ECHO NEXT 已经有很多工程能力，但播放器要真正变得好看、清晰、有记忆点，不能只靠程序员硬调 CSS。无论你擅长应用界面、品牌视觉、图标、封面风格、角色视觉、动效、宣传图，还是能帮我们判断“这个界面到底像不像人能看的东西”，都非常欢迎申请。不会写代码也可以申请；有可展示作品、审美判断和持续沟通意愿就很有价值。

开发者计划不只招代码贡献者。我们正在寻找：

| 方向 | 可以参与的内容 |
| --- | --- |
| 前端 / 交互 | 播放器页面、歌词、MV、曲库、设置、插件界面和整体体验打磨 |
| 后端 / 原生 / 音频 | Electron 主进程、曲库扫描、SQLite、WASAPI / ASIO、Linux 适配和诊断工具 |
| 美工 / 视觉设计 | UI 视觉、图标、插画、品牌素材、动效、截图宣传图和整体视觉一致性 |
| 测试 / 文档 | 功能验证、复现问题、用户教程、开发文档、Linux 构建和插件文档 |

申请时请准备 **GitHub、QQ 和开发经验**；如果你申请美工或设计方向，请额外附上作品集、截图、站点、Pixiv / Behance / ArtStation / Bilibili / 小红书等可展示链接，或直接说明你擅长的视觉方向。为 ECHO 参与过仓库贡献的用户可以直接申请，附上 QQ 和 GitHub ID 即可通过。

> [!NOTE]
> 云服务推荐：[雨云](https://www.rainyun.com/NzY3Mzg5_) 是面向开发者、站长和个人用户的新一代云服务提供商，提供云服务器、游戏云、物理服务器、虚拟主机、对象存储等产品。无论是部署网站、后端服务、开发测试环境，还是搭建游戏服务器都很适合使用雨云；如果你想开 Minecraft 高性能服务器、联机服、模组服或其他需要稳定网络与算力的游戏服务，也推荐优先考虑雨云。
...
如果你想更深入参与 ECHO NEXT，可以申请加入 [ECHO 开发者计划](https://echonext.moe/zh/developer/)。这不是普通的“点个 Star 支持一下”入口，而是给愿意实际参与项目建设的人准备的长期协作计划。

通过开发者申请后，你可以访问开发者仓库，参与更早期的功能讨论、开发和测试；符合条件的开发者也可以免费获得 ECHO Next Pro 等权益。当前尤其欢迎以下方向：

- 能写代码的人：前端、Electron、Node、TypeScript、原生音频、SQLite、Linux、插件 SDK、构建和发布流程都可以参与。
- 能做设计的人：我们非常需要美工、UI 视觉、图标、插画、角色视觉、动效和宣传素材方向的协作者。
- 能认真测试和写文档的人：复现问题、整理教程、补充平台说明、帮新用户把复杂功能看懂，也同样重要。

所有为 ECHO 参与过仓库贡献的用户可以直接申请；申请时附上 QQ 和 GitHub ID 即可通过。
...
| 美工 / 设计作品 | 如果申请视觉方向，请附上作品集、截图、主页链接或你擅长的视觉范围 |
